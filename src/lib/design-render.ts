...
export interface DesignLayer {
  ...
  // ── 텍스트 ──
  text?: string;
  /** 글자 크기 — 이미지 가로 대비 비율. 이래야 어떤 해상도에서도 같게 보인다 */
  size?: number;
  weight?: number;
  /** 자간 (em) */
  tracking?: number;
...
export function renderLayersToSvg(design: DesignDoc, W: number, H: number): string {
  ...
  design.layers.forEach((l, i) => {
    ...
    if (l.kind === 'icon') {
      ...
      if (!ic) return;
      // 24x24 좌표계를 원하는 크기로 키운다. size 는 이미지 가로 대비 비율
      const box = (l.size ?? 0.06) * W;
      const k = box / 24;
