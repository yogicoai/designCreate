...
${c.spec}`}
          ...
          */}
          ...
          <Section n={secNo()} title="배경 · 연출" hint="배경 공간·조명·분위기를 한글로 편하게 적으면 됩니다. 배경 사진을 올렸다면 그 공간에 더할 것만 적으세요.">
            {/*
              인물 연출 한 번에 — 얼굴 시트·표정컷이 전부 정면이라 그냥 두면 둘 다 카메라를 보고 나란히 웃는다
              (사용자 지적 2026-09-23 "자연스럽게 대화하는 연출이면 더 좋은데"). 방향 지시는 프롬프트에서 가장 우선이라 확실히 먹힌다.
            */}
            {withPeople && picks.length > 0 && (
              <div className="mb-2">
                <div className="text-[11px] mb-1" style={{ color: 'var(--text-dim)' }}>인물 연출 (누르면 아래 지시문에 들어갑니다):</div>
                <div className="flex flex-wrap gap-1.5">
                  {(picks.length > 1
                    ? [
                        ['서로 대화', '두 사람이 서로 마주 보며 이야기하는 순간 — 한 사람은 말하고 다른 사람은 웃으며 듣는다. 카메라는 보지 않는다'],
                        ['같은 곳 보기', '두 사람이 같은 곳(창밖·벽난로·TV)을 함께 바라보며 편하게 앉아 있다. 카메라는 보지 않는다'],
                        ['한 명만 카메라', '한 사람만 카메라를 보고 웃고, 다른 사람은 그 사람을 바라본다'],
                        ['둘 다 카메라', '두 사람 모두 카메라를 보며 자연스럽게 웃는다'],
                      ]
                    : [
                        ['혼자 쉬는 순간', '카메라를 의식하지 않고 제품에 편하게 기대 쉬는 순간 — 시선은 창밖이나 손에 든 것으로'],
                        ['책·컵 들고', '책을 보거나 머그를 든 채 제품에 기대 있는 자연스러운 순간'],
                        ['카메라 보기', '카메라를 보며 자연스럽게 웃는다'],
                      ]
                  ).map(([label, hint]) => (
